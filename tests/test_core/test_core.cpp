...
int main() {
    ...
    char ch = 'Z';
    std::string const res_char = format("Char: {}", ch);
    assert(res_char == "Char: Z");
    std::cout << "  \u2713 Single char: " << res_char << '\n';

    // Test booleans
    std::string const res_bool = format("Bools: {} / {}", true, false);
    assert(res_bool == "Bools: true / false");
    std::cout << "  \u2713 Booleans: " << res_bool << '\n';

    // Test numbers (int, float, negative)
    std::string const res_nums = format("Int: {}, Float: {}, Neg: {}", 42, 3.1415, -100);
    ...
    std::string const res_ptr = format("Ptr: {}, Null: {}", static_cast<void*>(ptr), null_ptr);
    ...
    println("Simple println string");
    info("Info message");
    warn("Warn message");
    err("Err message");

    info("Formatted Info: User {} (ID: {}) logged in", "Alice", 1001);
    err("Formatted Err: Failed to connect to {}:{}", "127.0.0.1", 8080);
